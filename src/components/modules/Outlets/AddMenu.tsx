...
const AddMenu: React.FC<AddMenuItemProps> = ({ 
  open, 
  onOpenChange, 
  outletId,
  onSuccess 
  ...
  const onSubmit = async (values: z.infer<typeof menuItemSchema>) => {
    try {
      ...
      const payload = {
        ...values,
        outlet_id: outletId, // Use the provided outlet ID
      ...
      const response = await olfService.post('/dish', payload);
      ...
      toast.error(`Failed to add menu item! ${error instanceof Error ? error.message : 'Unknown error'}`, {
        style: {
          borderRadius: '10px',
          background: 'black',
          color: 'red',
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-green-800">Add New Menu Item</DialogTitle>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-green-700">Tax Percentage (%)</FormLabel>
                      <FormControl>
                        <Input 
                          type="number" 
                          min="0"
            ...
            <DialogFooter className="pt-4">
              ...
              >
                {isUploading ? 'Processing...' : 'Add Menu Item'}
              </Button>
            </DialogFooter>
          </form>
        </Form>
