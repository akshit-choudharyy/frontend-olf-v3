...
const NewItemReq = () => {
  ...
  const [outletsMap, setOutletsMap] = useState<Record<number, any>>({});

  useEffect(() => {
    const getdata = async () => {
      try {
        // Fetch outlets. Note: Removed strict filters to ensure we get names
        // even if the outlet status is different from the dish status.
        const res = await olfService.get("/restraunts");
        const rows = res.data.data?.rows || [];

        // 1. Create a Lookup Map { 1193: { outlet_name: "XYZ", ... } }
        const map: Record<number, any> = {};
        rows.forEach((outlet: any) => {
          map[outlet.outlet_id] = outlet;
        });
        setOutletsMap(map);

        // 2. Keep your existing logic for veg_only default
        if (rows.length > 0) {
          const vegOnlyValue = rows[0]?.veg_only;
  ...
  const {
    ...
    queryFn: () => {
      const params: any = { change_type: 0, verified: false };
      if (isVegOnly) {
  ...
  const filteredMenuItems = React.useMemo(() => {
    return allMenuItems?.filter((item: MenuItem) => {
      // 1. ADD THIS LINE: Filter for Status 3 (Pending)
      ...
      // 2. ADD matchesStatus TO THE RETURN STATEMENT
      return (
        matchesStatus &&
        matchesFoodType &&
        matchesSearch &&
  ...
  const handleApprove = async (itemId: number, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation(); // Stop clicking the row behind the button

    try {
      // Update status to 1 (Active) and change_type to 0 (No pending changes)
      await olfService.put(`/dish/${itemId}`, {
        status: 1,
        change_type: 0,
      });
  ...
  const handlePushAllToIRCTC = async () => {
    ...
    try {
      ...
      await olfService.post(
        `dish/${stationCode}/${outletId}`,
      ...
      toast.error(
        `IRCTC Update Failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        {
          style: {
            borderRadius: "10px",
            background: "wheat",
            ...
            <TabsContent value={activeTab} className="mt-0">
                  ...
                  <Table>
                    <TableHeader className="bg-green-100">
                      <TableRow className="hover:bg-green-100/80">
                        <TableHead className="font-bold text-green-800 border border-green-200">
                          ITEM ID
                        </TableHead>
                        <TableHead className="font-bold text-green-800 border border-green-200">
                          OUTLET INFO
                              ...
                              <TableCell className="border border-green-200">
                                <div className="flex flex-col text-sm">
                                  ...
                                  <div className="text-xs text-green-600">
                                    Vendor: {outletDetails.vendor_name || "N/A"}
                                  </div>
                                  <div className="text-xs text-gray-500 mt-1 bg-gray-100 px-1 py-0.5 rounded w-fit">
                                    Stn: {item.station_code || "N/A"}
                                  </div>
                                </div>
                              </TableCell>
                                    ...
                                    item.image !== "" && (
                                      <div className="w-10 h-10 rounded-md overflow-hidden bg-green-50 flex-shrink-0">
                                        <img
                                          ...
                                          onError={(e) => {
                                            // Optional: Hide image if the URL fails to load (404)
                                            (
                              ...
                              <TableCell className="border border-green-200">
                                <div className="flex flex-col text-sm">
                                  <div className="flex items-center">
                                    ...
                                    <span className="font-medium">
                                      {formatDisplayName(item.food_type)}
                                    </span>
                                  </div>
                                  <div className="flex items-center mt-1">
                                    <Tag className="h-3 w-3 text-green-600 mr-1" />
                                    <span>
                                      {formatDisplayName(item.cuisine)}
                                    </span>
                                  </div>
                                </div>
                              </TableCell>
                                ...
                                <div className="flex space-x-2 justify-center">
                                  ...
                                  <DropdownMenu>
                                    ...
                                    >
                                      <DropdownMenuItem
                                        onClick={(e) =>
                                          handleDeleteMenuItem(item.item_id, e)
                                        }
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="text-sm text-green-700">
                  ...
                  of {filteredMenuItems.length} results
                </div>
